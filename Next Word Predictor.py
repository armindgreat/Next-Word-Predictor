...
def predict_next_word(input_text, n_best):
    ...
    for i, word in enumerate(input_text.split()):
        try:
            ...
            print("Sorry! we couldn't suggest words based on your input!!!")
            ch = input("Would you like to add the input to dataset for better word suggestion (y/n): ")
            if(ch=="y"):
                with open("joined_text.txt", "a", encoding="utf-8") as f:
                    f.seek(0)
                    f.write(user_input + " ")
            else:
                print("Bye!!")  
...
possible = predict_next_word(user_input, 5)
